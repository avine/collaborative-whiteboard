...
export class CwCutComponent implements OnInit, OnDestroy {
  ...
  constructor(private service: CwService, private changeDetectorRef: ChangeDetectorRef) {}
  ...
  private subscribeToCutLength() {
    this.subscription = combineLatest([this.service.historyCutLength$, this.service.cutRange$]).subscribe(
      ([cutLength, [from, to]]) => {
